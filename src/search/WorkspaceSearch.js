...
export class WorkspaceSearch {
  ...
  constructor(workspace) {
     ...
     * @private
     */
    this.textInputPlaceholder_ = 'hello I am finding';
  ...
  getBlockCategories_() {
    ...
    for (let i = 0; i < toolbox.contents.length; i++) {
      if (toolbox.contents[i].name)
        blockCategories.push(toolbox.contents[i].name);
    }
    return blockCategories;
  }

  createBlockDropdown_() {
    const blockDropdown = document.createElement('select');
    const blockCategories = this.getBlockCategories_();
    blockCategories.forEach((blockCategory) => {
      const tempOpt = document.createElement('option');
      tempOpt.text = blockCategory;
      blockDropdown.add(tempOpt);
    })
